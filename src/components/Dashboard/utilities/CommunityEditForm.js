...
import { EDIT_COMMUNITY } from "../../../actions/types";
...
function CommunityEditForm(props) {
  const { params, token, userId } = props;
  const classes = useStyles();
  const [id, setId] = useState(params[0].id);
  const [state, setState] = useState(params[0].state);
  const [country, setCountry] = useState(params[0].country);
  const [countryList, setCountryList] = useState([]);
  const [stateList, setStateList] = useState([]);
  const [loading, setLoading] = useState(false);
  const [communityType, setCommunityType] = useState(params[0].communityType);
  const [communityCode, setCommunityCode] = useState(params[0].communityCode);
  const [name, setName] = useState(params[0].name);
  const [description, setDescription] = useState(params[0].description);
  const [address, setAddress] = useState(params[0].address);
  const [contactPerson, setContactPerson] = useState(params[0].contactPerson);
      ...
      const createForm = async () => {
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulEditSnackbar(
            `${response.data.data.data.name} Community is updated successfully!!!`
          );
          props.renderCommunityEdittedUpdateCounter();
